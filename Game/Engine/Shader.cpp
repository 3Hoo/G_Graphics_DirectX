...
void Shader::CreateGraphicsShader(const wstring& path, ShaderInfo info, ShaderArg arg)
{
	...
	_graphicsPipelineDesc.PrimitiveTopologyType = GetTopologyType(info.topology); // topology를 받아 topology type을 직접 찾기
...
void OutputShaderErrorMessage(ComPtr<ID3DBlob> errorMessage, HWND hwnd)
{
...
void Shader::CreateShader(const wstring& path, const string& name, const string& version, ComPtr<ID3DBlob>& blob, D3D12_SHADER_BYTECODE& shaderByteCode)
{
		...
		, name.c_str(), version.c_str(), compileFlag, 0, &blob, &_errBlob)))
	{
		...
		OutputShaderErrorMessage(_errBlob, nullptr);
	}
