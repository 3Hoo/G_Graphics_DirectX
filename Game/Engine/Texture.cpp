...
void Texture::Load(const wstring& path)
{
	...
	// 기존의 command list는 command queue에서 render begin때reset,  쌓아놓고 render end 일 때 한 번에 전송
	// 그런데 텍스처 등의 리소스들은 render begin과 render end 사이에 전송할 필요가 없다
	// 즉, 리소스를 로드하는 용도로 command list를 command queue에 새로 만들자!
	// (command queue는 하나만 있어야 하지만, command list는 여러개 있어도 됨)
