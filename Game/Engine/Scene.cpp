...
void Scene::Awake()
{
	// shared_ptr 특성상 reference를 받으면 reference count++가 된다
	// 하지만 const로 받으면 그런 일이 없으므로, 성능 문제 상으로 const를 붙이는 것이 더 좋음
...
{
	shared_ptr<Camera> mainCamera = _cameras[0];
	mainCamera->Render_Forward();	// 메인 카메라만 forward rendering
