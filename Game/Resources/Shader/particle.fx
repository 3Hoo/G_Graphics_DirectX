...
[maxvertexcount(6)]
// 전체 삼각형이 아닌 점 하나만 넣어서 그려야 할지 안 그려야 할지 판단한다
...
// CS에서 모두가 공유하는 하나의 공간
RWStructuredBuffer<ComputeShared> g_shared : register(u1);

// CS_Main
// g_vec2_1 : DeltaTime / AccTime
// g_int_0  : Particle Max Count
// g_int_1  : AddCount -> gpu에게 파티클을 2개 만들어라 -> AddCount=2, alive=0인 애들이 새로 만들어질 후보가 됨.
//                        그런데 병렬로 작동하므로, 동기화가 필요
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
    ...
    g_shared[0].addCount = addCount;    // 병렬적으로 작동하므로, 여러 파티클이 동시에 이걸 작동시킬 수 있다
    ...
    if (g_particle[threadIndex.x].alive == 0)
    {
        while (true)
        {
            ...
            int originalValue;
            // 동기화 (일종의 선착순 경합을 벌이는 것)
            // 이 함수는 한 번에 한 번만 호출된다
            /*
